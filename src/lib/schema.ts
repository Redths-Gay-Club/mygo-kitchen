...
const PlayerProfileSchema = z.object({ name: z.string(), score: z.number() });
...
const S2CUpdateScoreSchema = z.object({
    action: z.literal("update_score"),
    player: z.string(),
    score: z.number(),
});
export type S2CUpdateScore = z.infer<typeof S2CUpdateScoreSchema>;
...
const S2CErrorSchema = z.object({
    action: z.literal("error"),
    message: z.string(),
});
export type S2CError = z.infer<typeof S2CErrorSchema>;

export const S2CPacketSchema = z.union([S2CRoomInfoSchema, S2CChangeStageSchema, S2CAddPlayerSchema, S2CRemovePlayerSchema, S2CChangeJudgeSchema, S2CUpdateScoreSchema, S2CJudgeSelectCardSchema, S2CErrorSchema]);
export type S2CPacket = z.infer<typeof S2CPacketSchema>;

//c2s

const C2SJoinSchema = z.object({
    action: z.literal("join"),
    name: z.string(),
});
export type C2SJoin = z.infer<typeof C2SJoinSchema>;
...
export const C2SPacketSchema = z.union([
    C2SJoinSchema,
    C2SStartSchema,
...
export function rawParseObj<T, U extends z.ZodTypeDef, V>(schema: z.ZodType<T, U, V>, input: any) {
    try {
