...
import type { C2SChooseBestCard, C2SChooseCard, C2SChooseSentence, C2SPacket, C2SSelectCard, Card, PlayerProfile, S2CPacket, Screen } from "$lib/schema";

export const allOpenedRooms = new Map<string, Room>();
...
function generateNewRoomId(): string {
    while (true) {
        ...
        if (!allOpenedRooms.has(roomId)) return roomId;
    }
}
...
function judgeChoseBestCard(judge: Player, room: Room, packet: C2SChooseBestCard, stage: ServerWaitForBestCard) {
    ...
    setTimeout(() => startAgain(room, findNextJudge(room)), 5000);
}
...
function findNextJudge(room: Room): string {
